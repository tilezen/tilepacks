...
# def fetch_tile(x, y, z, typ, layer, format, api_key):
def fetch_tile(format_args):
    sleep_time = 0.5 * random.uniform(1.0, 1.7)
    response_info = []
    while True:
        url = 'https://tile.mapzen.com/mapzen/{typ}/v1/512/{layer}/{zoom}/{x}/{y}.{fmt}?api_key={api_key}'.format(**format_args)
        try:
            start = time.time()
...
def build_tile_packages(min_lon, min_lat, max_lon, max_lat, min_zoom, max_zoom,
        typ, layer, tile_format, output, output_formats, api_key, concurrency):

    fetches = []
    for x, y, z in mercantile.tiles(min_lon, min_lat, max_lon, max_lat, range(min_zoom, max_zoom + 1)):
        fetches.append(dict(x=x, y=y, zoom=z, typ=typ, layer=layer, fmt=tile_format, api_key=api_key))
